                        ...
                        @forelse ($trackings as $key => $tracking)
                            @if (!empty($tracking->trackable))
                                <tr>
                                    ...
                                    <td>{{ ($tracking->item) ? $tracking->item->name : 'PROD-'.$tracking->item_id }}</td>

                                    @if ($tracking->trackable_type == 'App\OrderItem')
                                        <td>-</td>
                                        <td>{{ $tracking->trackable->quantity }}</td>
                                        <td>Sales</td>
                                    @elseif ($tracking->trackable_type == 'App\ItemReceiving')
                                        <td>{{ $tracking->trackable->quantity }}</td>
                                        <td>-</td>
                                        <td>Receiving</td>
                                    @else
            ...
            <div class="col-md-4">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <form action="{{ url('inventory/tracking') }}" method="GET">
